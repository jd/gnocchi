...
def get_pagination_options(params, max_limit, default):
    ...
    try:
        limit = api.PositiveNotNullInt(limit)
    except ValueError:
...
def set_resp_link_hdr(resp, path, marker, *args):
    ...
    resp.add_link('<%s?%s>' % (path, params), "next")
...
class MetricsResource(object):
    ...
    def on_get(self, req, resp):
        kwargs = self.MetricListSchema(req.params)
        ...
        pagination_opts = get_pagination_options(
            kwargs, self.max_limit, api.METRIC_DEFAULT_PAGINATION)
        attr_filters = []
        ...
        if metrics and len(metrics) >= pagination_opts['limit']:
            set_resp_link_hdr(resp, req.app,
                              str(metrics[-1].id), kwargs, pagination_opts)
    ...
    def on_get(self, req, resp, metric_id):
        ...
        if granularity is not None:
            try:
                ...
                abort(400, "Invalid value for granularity")
