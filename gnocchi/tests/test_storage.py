...
class TestStorageDriver(tests_base.TestCase):
    ...
    def test_get_measures_aggregated(self):
        self.incoming.add_measures(self.metric.id, [
        ...
        for i, (agg, aggts) in enumerate(six.iteritems(measures)):
            self.assertEqual(agg, aggts.aggregation)
            self.assertEqual(aggregations[i], aggts.aggregation)
