...
class StorageDriver(object):
    ...
    def get_measures(self, metric, aggregations,
        ...
        return {
            aggmethod: list(itertools.chain(
                *[[(timestamp, timeseries[agg].sampling, value)
                   for timestamp, value
    ...
    def _get_measures_and_unserialize(self, metric, keys):
        if not keys:
            return []
        ...
        for key, raw in six.moves.zip(keys, raw_measures):
            try:
                ...
                LOG.error("Data corruption detected for %s "
                          ...
                          metric.id, key.aggregation_method, key.sampling, key)
            else:
                results.append(ts)
        return results
    ...
    def _get_measures_timeserie(self, metric, aggregation, keys,
        ...
        timeseries = self._get_measures_and_unserialize(metric, keys)
